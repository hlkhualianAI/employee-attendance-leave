...
type EmployeeRow = mysql.RowDataPacket & {
  id: number;
  userId: number | null;
...
try {
  ...
  if (rows.length === 0) {
      ...
      JSON.stringify(
        {
          ...
          migrated: 0,
          skipped: 0,
          message: "No employee records found; no Firestore writes performed.",
        },
        null,
        2
      )
    ...
    // Firestore limits a write batch to 500 operations.
