...
export async function getEmployees(userId?: number) {
  return (await records("employees"))
    ...
    .sort((a, b) => a.status.localeCompare(b.status) || a.fullName.localeCompare(b.fullName));
}
...
export async function getAttendanceByDate(workDate: string, userId?: number) {
  return (await attendanceWithEmployees())
    .filter(row => row.workDate === workDate && (userId === undefined || row.employee.userId === userId))
...
export async function getAttendanceByMonth(monthPrefix: string, userId?: number) {
  return (await attendanceWithEmployees())
    .filter(row => row.workDate.startsWith(monthPrefix) && (userId === undefined || row.employee.userId === userId))
    .sort((a, b) => a.workDate.localeCompare(b.workDate) || a.fullName.localeCompare(b.fullName))
    .map(row => ({ employeeCode: row.employeeCode, fullName: row.fullName, department: row.department, workDate: row.workDate, checkInAt: row.checkInAt, checkOutAt: row.checkOutAt, lateMinutes: row.lateMinutes, checkInMode: row.checkInMode, note: row.note }));
}
...
export async function getLeaveRequestsByMonth(monthPrefix: string, userId?: number) {
  return (await leaveWithEmployees())
    .filter(row => row.startDate.startsWith(monthPrefix) && (userId === undefined || row.employee.userId === userId))
    .sort((a, b) => a.startDate.localeCompare(b.startDate) || a.fullName.localeCompare(b.fullName))
    .map(row => ({ employeeCode: row.employeeCode, fullName: row.fullName, department: row.department, leaveType: row.leaveType, startDate: row.startDate, endDate: row.endDate, totalDays: row.totalDays, reason: row.reason, status: row.status }));
}
...
export async function getDashboardSummary(monthPrefix: string, userId?: number) {
  ...
  const scopedAttendance = attendanceRows.filter(row => row.workDate.startsWith(monthPrefix) && (userId === undefined || row.employee.userId === userId));
  const scopedLeave = leaveRows.filter(row => userId === undefined || row.employee.userId === userId);
  return {
...
export async function checkInEmployee(employeeId: number, workDate: string, timestamp: number, input: { checkInMode: "office" | "offsite"; latitude: number; longitude: number; deviceId: string; recordedByUserId: number; note?: string }) {
  const db = await getDb();
...
export async function createLeaveRequest(input: { employeeId: number; leaveType: "annual" | "sick" | "personal" | "other"; startDate: string; endDate: string; totalDays: number; reason?: string }) {
  const db = await getDb();
...
export async function seedDemoData() {
  ...
  const demoEmployees = [
    { employeeCode: "EMP-001", fullName: "สมชาย ใจดี", department: "ฝ่ายขาย", position: "Sales Executive" },
    { employeeCode: "EMP-002", fullName: "สมหญิง พรประเสริฐ", department: "ฝ่ายบุคคล", position: "HR Officer" },
    { employeeCode: "EMP-003", fullName: "ธนกร ศรีสุข", department: "ฝ่ายบัญชี", position: "Accountant" },
  ];
